...
// Map program tags to program names
const programTagMap: Record<string, string> = {
...
export default function ProgramTestimonials({ programTag }: { programTag: string }) {
  const programName = programTagMap[programTag] || programTag;

  // All testimonials (filtered by program)
  ...
  // Filter testimonials by program
  const testimonials = useMemo(
    () => allTestimonials.filter((t) => t.program === programName),
    [allTestimonials, programName]
  );

  // If no testimonials for this program, show all
  const displayTestimonials = testimonials.length > 0 ? testimonials : allTestimonials.slice(0, 3);

  return (
    <section className="py-8 sm:py-12" style={{ backgroundColor: white }}>
      <div className="max-w-6xl mx-auto px-4">
        <h2 className="text-2xl sm:text-3xl font-semibold mb-8" style={{ fontFamily: "'Poppins', sans-serif", color: black }}>
          What people say
        </h2>
